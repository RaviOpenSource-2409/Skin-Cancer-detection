...
# ── Pure NumPy forward pass (no tensorflow needed) ────────
@st.cache_resource
def load_weights():
...
def get(name):
    for k, v in W.items():
        if k.endswith("/" + name + "/kernel") or k.endswith("/" + name):
            return v
    # search by layer name suffix
    matches = [v for k, v in W.items() if ("/" + name + "/") in k]
    return matches
...
def forward(img_norm):
    x = img_norm
    # Conv block 1
    x = relu(conv2d_same(x,
        get_layer("conv2d_80","kernel"), get_layer("conv2d_80","bias")))
    x = maxpool2x2(x)
    x = bn(x, get_layer("batch_normalization_80","gamma"),
              get_layer("batch_normalization_80","beta"),
              get_layer("batch_normalization_80","moving_mean"),
              get_layer("batch_normalization_80","moving_variance"))
    # Conv block 2
    x = relu(conv2d_same(x,
        get_layer("conv2d_81","kernel"), get_layer("conv2d_81","bias")))
    x = relu(conv2d_same(x,
        get_layer("conv2d_82","kernel"), get_layer("conv2d_82","bias")))
    x = maxpool2x2(x)
    x = bn(x, get_layer("batch_normalization_81","gamma"),
              get_layer("batch_normalization_81","beta"),
              get_layer("batch_normalization_81","moving_mean"),
              get_layer("batch_normalization_81","moving_variance"))
    # Conv block 3
    x = relu(conv2d_same(x,
        get_layer("conv2d_83","kernel"), get_layer("conv2d_83","bias")))
    x = relu(conv2d_same(x,
        get_layer("conv2d_84","kernel"), get_layer("conv2d_84","bias")))
    x = maxpool2x2(x)
    # Dense layers
    x = x.flatten()
    x = relu(x @ get_layer("dense_66","kernel") + get_layer("dense_66","bias"))
    x = bn(x, get_layer("batch_normalization_82","gamma"),
              get_layer("batch_normalization_82","beta"),
              get_layer("batch_normalization_82","moving_mean"),
              get_layer("batch_normalization_82","moving_variance"))
    x = relu(x @ get_layer("dense_67","kernel") + get_layer("dense_67","bias"))
    x = bn(x, get_layer("batch_normalization_83","gamma"),
              get_layer("batch_normalization_83","beta"),
              get_layer("batch_normalization_83","moving_mean"),
              get_layer("batch_normalization_83","moving_variance"))
    x = relu(x @ get_layer("dense_68","kernel") + get_layer("dense_68","bias"))
    x = bn(x, get_layer("batch_normalization_84","gamma"),
              get_layer("batch_normalization_84","beta"),
              get_layer("batch_normalization_84","moving_mean"),
              get_layer("batch_normalization_84","moving_variance"))
